...
class Database {
  ...
  static Future<String?> addTracker(String uid, Tracker tracker) async {
    try {
      await _db.collection('users').doc(uid).collection('trackers').add({
        'name': tracker.name,
        'med_ids': tracker.med_ids,
        'med_names': tracker.med_names,
        'tracker_time': tracker.tracker_time,
      }).then((result) {
        return 'success';
      });
    } catch (e) {
  ...
  static Future<List<Tracker>> getTrackers(String uid) async {
    final QuerySnapshot snapshot =
    await _db.collection('users').doc(uid).collection('trackers').get();
    return snapshot.docs.map((doc) => Tracker.fromDocument(doc)).toList();
  }

  static Future<String?> deleteTracker(String uid, String id) async {
    try {
      await _db
          .collection('users')
          .doc(uid)
